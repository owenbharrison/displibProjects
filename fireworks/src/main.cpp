...
struct Particle {
	...
	Particle(V2D pos_, V2D vel_, float lifeSpan_) {
		pos=pos_;
		vel=vel_;
		lifeSpan=lifeSpan_;
	}
...
struct Firework {
	...
	Firework(V2D pos_, V2D vel_, short col_, int numPtc_, float maxRad_, float minLifeSpan_, float maxLifeSpan_) {
		pos=pos_;
		vel=vel_;
		col=col_;
		numPtc=numPtc_;
	...
	void blowUp() {
		...
		if (!blown) {
			for (int i=0; i<numPtc; i++) {
				...
				particles.push_back(Particle(pos, V2D::fromAngle(angle)*rad, lifeSpan));
			}
...
class Demo : public Engine {
	...
	void update(float dt) override {
		...
		if (timer>nextFWTime) {
			...
			fireworks.push_back(Firework(V2D(x, height), V2D(xv, yv), col, num, width/12, 1.3f, 2.7f));
		}
