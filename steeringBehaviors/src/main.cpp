...
struct particle {
	V2D pos, vel, acc, target;
	float maxSpeed=400, maxForce=30;
	short r, g, b;

	particle(V2D pos_, V2D target_, int r_, int g_, int b_) {
		pos=pos_;
		target=target_;
		r=r_;
		g=g_;
...
class Demo : public Engine {
		...
		for (int x=1; x<width-1; x++) {
			for (int y=1; y<height-1; y++) {
				...
				if (!left||!right||!top||!bottom) {
					...
					particles.push_back(particle(V2D(rx, ry), V2D(x, y), r, g, b));
				}
			}
		}
		delete[] newData;
	...
	void update(float dt) override {
		...
		for (particle& p:particles) {
			...
			//flee mouse
			if ((mp-p.pos).mag()<7) p.applyForce(p.flee(mp)*50);
			//update
			p.update(dt);
		}
